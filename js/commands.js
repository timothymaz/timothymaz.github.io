var youtube = "N/A";
var twitter = "N/A";
var password = "NeoTrinity";
var linkedin = "https://www.linkedin.com/in/timothy-mazur-b3bb4217a/";
var instagram = "https://www.instagram.com/tim.mazur";
var github = "https://github.com/timothymaz";
var email = 'mailto:timothymazur125@gmail.com';

whois = [
  "<br>",
 "Hey I’m Tim,",
"I am a Cybersecurity professional with a passion for protecting people and organizations from",
"online threats. With 1 year of experience in the field, I have a strong background in various",
"areas of cybersecurity, including penetration testing, incident response, and threat intelligence.",
"I hold the CompTIA A+, Network+, and Security +. I am particularly interested in the areas",
"of threat hunting and incident response, and I am always looking for new ways to",
"stay up-to-date with the latest technology.",
"In my current role as Cyber security Consultant, I protect our clients from cyber-attacks.",
"Furthermore, I am dedicated to staying current with the latest trends and techniques in the field,",
"and actively seeking out opportunities to share my knowledge and experience with others.",
"When I'm not working, I enjoy flying FPV drones, Photography, and going to the gym.",
"I believe in the power of the sharing of knowledge and continuous learning,",
"and I hope to inspire others in the cybersecurity field to strive for excellence and",
"make a positive impact in the world.",
  "<br>"
];

whoami = [
  "<br>",
  "The paradox of “Who am I?” is: we never know, but, we constantly find out.",
  "<br>"
];

social = [
  "<br>",
//'youtube        <a href="' + youtube + '" target="_blank">youtube/fknight' + "</a>",
//'twitter        <a href="' + twitter + '" target="_blank">twitter/forrestpknight' + '</a>',
  'linkedin       <a href="' + linkedin + '" target="_blank">linkedin/TimothyMazur' + "</a>",
  'instagram      <a href="' + instagram + '" target="_blank">instagram/Tim.Mazur' + '</a>',
  'github         <a href="' + github + '" target="_blank">github/timothymaz' + "</a>",
  "<br>"
];

secret = [
  "<br>",
  '<span class="command">sudo</span>           Only use if you\'re admin',
  "<br>"
];

projects = [
  "<br>",
  "Still curating... most projects are offline, on GitHub, or confidential.",
  "<br>"
];

help = [
  "<br>",
  '<span class="command">whois</span>          Who is Tim?',
  '<span class="command">whoami</span>         Who are you?',
//  '<span class="command">video</span>          View YouTube videos',
  '<span class="command">social</span>         Display social networks',
  '<span class="command">secret</span>         Find the password',
//  '<span class="command">projects</span>       View coding projects',
  '<span class="command">history</span>        View command history',
  '<span class="command">help</span>           You obviously already know what this does',
  '<span class="command">email</span>          Do not email me',
  '<span class="command">clear</span>          Clear terminal',
  '<span class="command">banner</span>         Display the header',
  "<br>",
];

banner = [
  '<span class="index"></span>',
  "",

  "TTTTTTTTTTTTTTTTTTTTTTT  iiii                               MMMMMMMM               MMMMMMMM                                                                         ",
  "T:::::::::::::::::::::T i::::i                              M:::::::M             M:::::::M                                                                         ",
  "T:::::::::::::::::::::T  iiii                               M::::::::M           M::::::::M                                                                         ",
  "T:::::TT:::::::TT:::::T                                     M:::::::::M         M:::::::::M                                                                         ",
  "TTTTTT  T:::::T  TTTTTTiiiiiii    mmmmmmm    mmmmmmm        M::::::::::M       M::::::::::M  aaaaaaaaaaaaa   zzzzzzzzzzzzzzzzzuuuuuu    uuuuuu  rrrrr   rrrrrrrrr   ",
  "        T:::::T        i:::::i  mm:::::::m  m:::::::mm      M:::::::::::M     M:::::::::::M  a::::::::::::a  z:::::::::::::::zu::::u    u::::u  r::::rrr:::::::::r  ",
  "        T:::::T         i::::i m::::::::::mm::::::::::m     M:::::::M::::M   M::::M:::::::M  aaaaaaaaa:::::a z::::::::::::::z u::::u    u::::u  r:::::::::::::::::r ",
  "        T:::::T         i::::i m::::::::::::::::::::::m     M::::::M M::::M M::::M M::::::M           a::::a zzzzzzzz::::::z  u::::u    u::::u  rr::::::rrrrr::::::r",
  "        T:::::T         i::::i m:::::mmm::::::mmm:::::m     M::::::M  M::::M::::M  M::::::M    aaaaaaa:::::a       z::::::z   u::::u    u::::u   r:::::r     r:::::r",
  "        T:::::T         i::::i m::::m   m::::m   m::::m     M::::::M   M:::::::M   M::::::M  aa::::::::::::a      z::::::z    u::::u    u::::u   r:::::r     rrrrrrr",
  "        T:::::T         i::::i m::::m   m::::m   m::::m     M::::::M    M:::::M    M::::::M a::::aaaa::::::a     z::::::z     u::::u    u::::u   r:::::r            ",
  "        T:::::T         i::::i m::::m   m::::m   m::::m     M::::::M     MMMMM     M::::::Ma::::a    a:::::a    z::::::z      u:::::uuuu:::::u   r:::::r            ",
  "      TT:::::::TT      i::::::im::::m   m::::m   m::::m     M::::::M               M::::::Ma::::a    a:::::a   z::::::zzzzzzzzu:::::::::::::::uu r:::::r            ",
  "      T:::::::::T      i::::::im::::m   m::::m   m::::m     M::::::M               M::::::Ma:::::aaaa::::::a  z::::::::::::::z u:::::::::::::::u r:::::r            ",
  "      T:::::::::T      i::::::im::::m   m::::m   m::::m     M::::::M               M::::::M a::::::::::aa:::az:::::::::::::::z  uu::::::::uu:::u r:::::r            ",
  "      TTTTTTTTTTT      iiiiiiiimmmmmm   mmmmmm   mmmmmm     MMMMMMMM               MMMMMMMM  aaaaaaaaaa  aaaazzzzzzzzzzzzzzzzz    uuuuuuuu  uuuu rrrrrrr   ",
 
 
                                                                                                           
 
 
  '<span class="color2">Welcome to my interactive web terminal.</span>',
  "<span class=\"color2\">For a list of available commands, type</span> <span class=\"command\">'help'</span><span class=\"color2\">.</span>",
];